...
cvar_t m_accelerate_natural_strength = {CF_CLIENT | CF_ARCHIVE, "m_accelerate_natural_strength", "0", "How quickly the accelsensitivity approaches the m_accelerate_natural_accelsenscap value values are compressed between 0 and 1 but higher numbers are allowed"};
cvar_t m_accelerate_natural_accelsenscap = {CF_CLIENT | CF_ARCHIVE, "m_accelerate_natural_accelsenscap", "0", "Horizontal asymptote that sets the maximum value for the natural mouse acceleration curvel, value 2, for example, means that the maximum sensitivity is 2 times the base sensitivity"};
	...
	if(m_accelerate.value > 0)
	{
		...
		if (m_accelerate_power_strength.value != 0.0f)
		{
			...
			in_mouse_y *= accelsens;
                ...
                if (m_accelerate_natural_strength.value > 0.0f && m_accelerate_natural_accelsenscap.value > 1.0f)
                {
                    ...
                    if (adjusted_speed_pxms > 0)
                    {
                        ...
                        accelsens += (adjusted_accelsenscap - adjusted_accelsenscap * exp( - (pow(adjusted_speed_pxms, m_accelerate_natural_strength.value) / fabs(adjusted_accelsenscap) ))) * inv_sensitivity;
                    }
