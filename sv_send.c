...
void SV_StartParticle (vec3_t org, vec3_t dir, int color, int count)
{
	...
	sv.protocol->WriteCoord (&sv.datagram, org[0]);
	sv.protocol->WriteCoord (&sv.datagram, org[1]);
	sv.protocol->WriteCoord (&sv.datagram, org[2]);
	for (i=0 ; i<3 ; i++)
...
// directed messages go only to the entity they are targeted on
	...
	for (i = 0;i < 3;i++)
		sv.protocol->WriteCoord (&sv.datagram, origin[i]);
	SV_FlushBroadcastMessages();
}
